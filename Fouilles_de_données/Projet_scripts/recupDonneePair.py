...
for lineGene in donneesGene.readlines(): # Lecture de chaque ligne du fichier donneesGene.txt
	if first : # Première ligne
		...
		if pG[3] == lG[3]: # Si les deux gènes sont sur le même brin 
			...
			dist = int(lG[2]) - int(pG[1]) + 1 # Calcul de la distance entre les deux gènes
			...
			tailleDeuxGene = int(lG[2]) - int(lG[1]) + int(lG[2]) - int(lG[1])
			stringScore = moy
			
			#########################################################################################################
			# Récupération du score de String 
			donneesString = open("donneesString.txt", "r") # Ouverture du fichier donneesString.txt en lecture
			for lineString in donneesString.readlines(): # Lecture de chaque ligne du fichier donneesString.txt
				lS=lineString.split('	') # Création d'une liste de chaine de caractères 
				if (lS[0]== pG[0] and lS[1]== lG[0]) or (lS[0]== lG[0] and lS[1]== pG[0]): # Recherche de la paire correspondantes
					stringScore = float(lS[14]) # Récupération du score de String 
